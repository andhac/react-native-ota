...
/// Encode / decode `state.json` with stable key order.
public enum OtaStateCodec {
  public static func encode(_ state: BundleState) -> Data {
    var parts: [String] = []
    parts.append("\"schemaVersion\":\(state.schemaVersion)")
    parts.append("\"installedBinaryVersion\":\(jsonStringOrNull(state.installedBinaryVersion))")
    parts.append("\"activeSlot\":\(jsonStringOrNull(state.activeSlot))")
    parts.append("\"pendingSlot\":\(jsonStringOrNull(state.pendingSlot))")
    parts.append("\"previousSlot\":\(jsonStringOrNull(state.previousSlot))")
    let body = "{" + parts.joined(separator: ",") + "}\n"
    return Data(body.utf8)
  }

  public static func decode(bytes: Data, supportedSchemaVersion: Int) -> BundleState? {
    guard !bytes.isEmpty, let text = String(data: bytes, encoding: .utf8)?.trimmingCharacters(in: .whitespacesAndNewlines) else {
      return nil
    }
    guard let root = try? parseObject(text) else { return nil }
    guard let schemaRaw = root["schemaVersion"], let schemaVersion = Int(schemaRaw ?? "") else {
      return nil
    }
    ...
    return BundleState(
      ...
      installedBinaryVersion: nullableString(root, "installedBinaryVersion"),
      activeSlot: sanitizeSlotId(nullableString(root, "activeSlot")),
      pendingSlot: sanitizeSlotId(nullableString(root, "pendingSlot")),
      previousSlot: sanitizeSlotId(nullableString(root, "previousSlot"))
    )
  }

  public static func isValidSlotId(_ slotId: String) -> Bool {
    slotId.range(of: OtaPaths.slotIdPattern, options: .regularExpression) != nil
  }

  public static func sanitizeSlotId(_ raw: String?) -> String? {
    guard let raw, !raw.isEmpty else { return nil }
    return isValidSlotId(raw) ? raw : nil
  }
  ...
  private static func nullableString(_ root: [String: String?], _ key: String) -> String? {
    guard root.keys.contains(key) else { return nil }
    return root[key] ?? nil
  }
